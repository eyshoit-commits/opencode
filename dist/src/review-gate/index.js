...
export async function openReviewGate(input, options = {}) {
    const compatibility = loadAcpCompatibilityConfig();
    if (compatibility.reviewGateMode === "disabled")
        throw new Error("The review gate is disabled.");
    ...
    try {
        if (options.openBrowser !== false)
            await openReviewBrowser(url);
        const decided = await waitForReviewDecision(session.id, options.timeoutMs ?? reviewGateTimeoutMs(compatibility));
