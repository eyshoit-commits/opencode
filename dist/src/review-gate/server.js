...
import { addReviewAnnotation, createReviewSession, decideReviewSession, readReviewSession, } from "./session.js";
...
async function route(request, response) {
    ...
    if (request.method === "POST" && url.pathname === "/api/review/open") {
        sendJson(response, 201, await createReviewSession(await readJson(request)));
        return;
    }
