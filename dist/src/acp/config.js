...
export function loadAcpCompatibilityConfig(env = process.env) {
    const reviewGateMode = parseReviewGateMode(env.BKG_OC_REVIEW_GATE_MODE);
    return {
        enabled: parseBoolean(env.BKG_OC_ACP_ENABLED, true),
        dashboardPort: parseInteger(env.BKG_OC_DASHBOARD_PORT, DEFAULT_DASHBOARD_PORT, "BKG_OC_DASHBOARD_PORT", 1, 65535),
        remoteMode: parseBoolean(env.BKG_OC_DASHBOARD_REMOTE, false),
        browser: nonEmpty(env.BKG_OC_DASHBOARD_BROWSER),
        reviewGateMode,
        reviewGateTimeoutSeconds: parseInteger(env.BKG_OC_REVIEW_GATE_TIMEOUT_SECONDS, DEFAULT_REVIEW_TIMEOUT_SECONDS, "BKG_OC_REVIEW_GATE_TIMEOUT_SECONDS", 1, 30 * 24 * 60 * 60),
        unsupportedSlashCommands: ["/undo", "/redo"],
    };
}
export function reviewGateTimeoutMs(config = loadAcpCompatibilityConfig()) {
    return config.reviewGateTimeoutSeconds * 1000;
}
function parseReviewGateMode(value) {
    if (!value)
        return "browser";
    if (value === "browser" || value === "disabled")
        return value;
