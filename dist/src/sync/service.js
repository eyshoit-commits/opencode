...
export function createSyncService(config = defaultSyncConfig()) {
    return {
        async status() {
        ...
        async push(message = `Sync OpenCode config (${new Date().toISOString().slice(0, 10)})`) {
            ...
            for (const spec of specs) {
                if (spec.secret && !config.includeSecrets)
                    continue;
                if (await copyToRepo(repoPath, spec))
                    files.push(spec.id);
            }
            await fs.writeFile(path.join(repoPath, "sync-manifest.json"), JSON.stringify(buildSyncManifest(config), null, 2) + "\n", "utf8");
        ...
        async pull() {
            const repoPath = await ensureRepository(config);
            await git(repoPath, ["pull", "--ff-only"]);
            const manifest = JSON.parse(await fs.readFile(path.join(repoPath, "sync-manifest.json"), "utf8"));
            ...
            for (const remoteSpec of manifest.paths) {
                ...
                if (!localSpec || (remoteSpec.secret && !config.includeSecrets))
                    continue;
                if (await copyFromRepo(repoPath, localSpec, backupPath))
                    files.push(localSpec.id);
            }
            return { action: "pull", repoPath, changed: files.length > 0, files, backupPath };
        },
    };
}
function resolveLocalRepoPath(config) {
    if (!config.repo)
    ...
    if (configured)
        return path.resolve(expandHome(configured, roots.home));
    ...
    try {
        ...
        const remote = config.repo.remote ??
            (config.repo.owner && config.repo.name
                ...
                : undefined);
        if (!remote)
            throw new Error("Sync repo needs remote or owner/name.");
...
function repoItemPath(repoPath, spec) {
    return path.join(repoPath, "files", spec.id);
}
async function copyToRepo(repoPath, spec) {
    ...
    try {
        ...
        if (stat.isDirectory())
            await fs.cp(spec.path, destination, { recursive: true });
        else
            await fs.copyFile(spec.path, destination);
        return true;
    }
    catch (error) {
        if (spec.optional && isMissing(error))
            return false;
        throw error;
    }
}
async function copyFromRepo(repoPath, spec, backupPath) {
    ...
    try {
        ...
        try {
            ...
            if (currentStat.isDirectory())
                await fs.cp(spec.path, backup, { recursive: true });
            else
                await fs.copyFile(spec.path, backup);
        }
        catch (error) {
            if (!isMissing(error))
                throw error;
        ...
        if (sourceStat.isDirectory())
            await fs.cp(source, spec.path, { recursive: true });
        else
            await fs.copyFile(source, spec.path);
        return true;
    }
    catch (error) {
        if (spec.optional && isMissing(error))
