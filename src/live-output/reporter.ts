...
export function createLiveOutputReporter(): LiveOutputReporter {
  const rootSessionIds = new Set<string>()
  const sessions = new Map<string, SessionInfo>()
  const subagents = new Map<string, AgentInfo>()
  ...
  function isDescendant(sessionId: string): boolean {
    if (rootSessionIds.has(sessionId)) return true
    let cursor: string | undefined = sessionId
    const seen = new Set<string>()
    while (cursor && !seen.has(cursor)) {
      seen.add(cursor)
      const parentId: string | undefined = sessions.get(cursor)?.parentId
      if (!parentId) return false
      if (rootSessionIds.has(parentId)) return true
      ...
      if (!sessions.has(parentId)) return true
      cursor = parentId
    }
    return false
  }
  ...
  function depthOf(sessionId: string): number {
    ...
    while (cursor && !seen.has(cursor)) {
      seen.add(cursor)
      const parentId: string | undefined = sessions.get(cursor)?.parentId
      if (!parentId || rootSessionIds.has(parentId)) return depth + 1
      if (!sessions.has(parentId)) return depth + 1
      depth += 1
      cursor = parentId
    }
    return depth + 1
  ...
  function assign(sessionId: string): AgentInfo | undefined {
    if (rootSessionIds.has(sessionId)) return undefined
    const session = sessions.get(sessionId)
    if (!session?.parentId || !isDescendant(sessionId)) return undefined
    const agentName = titleCase(session.agent ?? "Subagent")
    const existing = subagents.get(sessionId)
    if (existing?.agentName === agentName) return existing
  ...
  return {
    handle(rawEvent: unknown): void {
      ...
      if (type === "session.created") {
        ...
        if (!parentId) rootSessionIds.add(id)
        return
      }
      ...
      if (!sessionId || !isDescendant(sessionId)) return
      ...
      if (part.type === "tool") {
        ...
        if (toolStatuses.get(partId) === status) return
        ...
        if (lastToolSignature.get(sessionId) === signature) return
...
function summarizeToolInput(tool: string, rawInput: unknown): string | undefined {
...
function getField(input: Record<string, unknown>, keys: string[]): string | undefined {
