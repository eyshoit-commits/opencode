...
export async function listBlockers(
  ...
  for (const file of files.filter((f: string) => f.endsWith(".json"))) {
    try {
      const b = JSON.parse(
        await fs.readFile(path.join(BLOCKERS_DIR, file), "utf8"),
      ) as BlockerRecord;
...
export async function autoStartRatForBlocker(
  blocker: BlockerRecord,
  agents: string[],
): Promise<BlockerRecord> {
  if (!shouldStartRat(blocker)) return blocker;
